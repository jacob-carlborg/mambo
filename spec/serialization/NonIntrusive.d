 ...
 */
module spec.serialization.NonIntrusive;
...
unittest
{
	...
	describe("serialize object using a non-intrusive method") in {
		it("should return a custom serialized object") in {
			...
			assert(archive.data().containsXmlTag("object", `runtimeType="spec.serialization.NonIntrusive.Foo" type="spec.serialization.NonIntrusive.Foo" key="0" id="0"`));
			assert(archive.data().containsXmlTag("int", `key="a" id="1"`, "3"));
			assert(archive.data().containsXmlTag("int", `key="b" id="2"`, "4"));
			
			assert(archive.data().containsXmlTag("base", `type="spec.serialization.NonIntrusive.Base" key="1" id="3"`));
